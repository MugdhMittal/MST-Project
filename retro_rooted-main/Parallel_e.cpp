...
public:
    ScopedTimer(const std::string &fName) : funcName(fName), start(Clock::now())
    {
        std::cerr << std::string(callDepth * 2, ' ')
                  << ">> Enter " << funcName << "\n";
        callDepth++;
    }
    ~ScopedTimer()
    {
        ...
        std::cerr << std::string(callDepth * 2, ' ')
                  << "<< Exit " << funcName << " (" << duration << " ms)\n";
    }
};

#define FUNC_TIMER ScopedTimer timerObj(__FUNCTION__)
...
public:
    ...
    void Process_Status(vector<Edge> &CE, vector<EdgeStatus> &Status, vector<Edge> &Marked)
    {
        ...
        vector<pair<Edge, Edge>> toReplace;
        vector<Edge> toInsertEx, toInsertEr;
        ...
        for (int i = 0; i < CE.size(); ++i)
        {
            ...
            case EdgeStatus::RPL:
                ...
                deletedNow.insert(Old.id); // old MST edge is effectively gone
                break;
            default:
                break;
            }
        ...
        // --- Apply in-place replacements first ---
        ...
        // Remove deleted edges from Ex and Er
        for (int did : deletedNow)
        {
            Ex.erase(did);
        }
        Er.erase(remove_if(Er.begin(), Er.end(),
                           [&](const Edge &e)
                           { return deletedNow.count(e.id); }),
                 Er.end());

        // Add new insertions directly
        ...
        if (Ex.size() < V - 1)
        {
            // Only rebuild missing parts
            vector<Edge> validEdges;
            for (auto &[id, e] : Ex)
                validEdges.push_back(e);
            validEdges.insert(validEdges.end(), Er.begin(), Er.end());
