...
public:
    ...
    void Process_Status(vector<Edge> &CE, vector<EdgeStatus> &Status, vector<Edge> &Marked)
    {
        ...
        for (int i = 0; i < (int)CE.size(); ++i)
        {
            ...
            case EdgeStatus::DEL:
            {
                ...
                // Search in Er if not found in Ex
                if (!found)
                {
                    for (Edge &e : Er)
                    {
    ...
    void Repair_Tree(vector<Edge> &Er, vector<EdgeStatus> &Status, vector<Edge> &Marked)
    {
        ...
        do
        {
            ...
            // This function is never really called. Just there for the worst case scenario
    ...
    void ProcessAllEdges(vector<Edge> &CE, vector<bool> &operation)
    {
        ...
        while (!CE.empty())
        {
            ...
            for (int i = 0; i < (int)CE.size(); ++i)
            {
                if (Status[i] != EdgeStatus::NONE &&
                    !deletedIds.count(CE[i].id) &&
                    std::none_of(Ex.begin(), Ex.end(), [&](const Edge &ex)
                                 { return ex.id == CE[i].id; }))
                {
